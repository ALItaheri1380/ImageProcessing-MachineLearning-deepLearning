function [image] =  My_Imresize_Euclidean_Distance(Input_Image, Resizing_Factor)
    ...
    for i = 1:new_n
       for j = 1:new_m
           ...
           ca = 1/(sqrt((x - floor(x))^2 + (y - floor(y))^2) + eps);
           cb = 1/(sqrt((x - floor(x))^2 + (ceil(y) - y)^2) + eps);
           cc = 1/(sqrt((ceil(x) - x)^2 + (y - floor(y))^2) + eps);
           cd = 1/(sqrt((ceil(x) - x)^2 + (ceil(y) - y)^2) + eps);
